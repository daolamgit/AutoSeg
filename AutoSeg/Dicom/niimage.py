...
class Patient(object):
    ...
    def __init__(self,file_path):
        ...
        self.Images_path, self.Structure_path   = None, None
        self.Slices, self.Volume                = None, None
        self.Volume_rescale                     = None
        #self.z_scale, self.inplane_scale = self.get_scale_xyz()

        self.Volume_resize                      = None #input to NN
        self.Hyper_volume                       = None  # 4D tensor
...
class Patient_Nii( Patient):
    def __init__(self, file_path):
        super( Patient_Nii, self).__init__(file_path)
    ...
    def find_images_contour_path(self):
        pt_name = os.path.basename( self.File_path)
        images_path = os.path.join( self.File_path, pt_name+'.nii.gz')
        structure_path = os.path.join( self.File_path ,'GT.nii.gz')
        return ( images_path, structure_path)

    def create_volume(self):
        slices    = nib.load( self.Images_path)
        images        = slices.get_fdata()
        #rotate images so it match my old code
        #but nii is a matlab thing even for x and y
        images      = np.swapaxes( images, -1, 0) #swapaxis change the order, transpose we need specify all axis
        images = self.normalize(images)
        return (slices, images)

    ###overide property method too
    @property
    def inplane_scale(self):
        inplane = {}
        PixelSpacing = abs( self.Slices.affine[0][0])

        nRows   = self.Slices.shape[0] #shape is 512, 512, 219 a matlab
        inplane['scale'] = PixelSpacing / PIXEL_SPACING
        inplane['size'] = int( np.rint( inplane['scale'] * nRows /2) * 2)
        return inplane

    @property
    def zplane_scale(self):
        zplane = {}
        SliceThickness = abs(self.Slices.affine[2][2])
        zplane['scale'] = SliceThickness /SLICE_THICKNESS
        zplane['size'] = int( np.rint( zplane['scale'] * self.Volume.shape[0]))
        return zplane
...
class Patient_Train(Patient):
    def __init__(self, file_path):
        ...
        self.Contours = None #self.get_contours()
        self.Masks = None #self.create_masks()  # 3D tensor
        self.Masks_rescale = None #self.get_masks_rescale()
        self.Masks_resize   = None #self.get_masks_resize()

    def driver(self):
        super(Patient_Train, self).driver()
...
class Patient_Train_Nii( Patient_Train, Patient_Nii):
    def __init__(self, file_path):
        super(Patient_Train_Nii, self).__init__(file_path)

    def driver(self):
        super(Patient_Train_Nii, self).driver()
        self.Contours = self.get_contours()
        self.Masks = self.create_masks()  # 3D tensor
        self.Masks_rescale = self.get_masks_rescale()
        self.Masks_resize   = self.get_masks_resize()
    ...
    def create_masks(self):
        structure = nib.load(self.Structure_path)
        masks = structure.get_fdata() #masks is an cardinal mask
        masks = np.swapaxes( masks, -1, 0)
        return masks
...
class Patient_Test_Nii( Patient_Nii):
    def __init__(self, file_path):
        super( Patient_Test_Nii, self).__init__(file_path)

    def driver(self):
        #hau qua cua driver. If
        self.Images_path, self.Structure_path   = self.find_images_contour_path()
        self.Slices, self.Volume                = self.create_volume()  # 3D tensor
        self.Volume_rescale                     = self.get_volume_rescale() #make every scan same physical size
        #self.z_scale, self.inplane_scale = self.get_scale_xyz()
    ...
    def find_images_contour_path(self):
        #overide when test folder has only iamge name not folder
        pt_name = os.path.basename( self.File_path)
        # pt_name = pt_name.split('.')[0]
        images_path = self.File_path#os.path.join( self.File_path, pt_name+'.nii.gz')
        structure_path = images_path.replace('.gz', '')#os.path.join( self.File_path ,'GT.nii.gz')
...
if __name__ == '__main__':
    ...
    #input_path_train    = r'../../Data/SegThor_Small/train'
    #input_path_test     = r'../../Data/SegThor_Small/test'
    input_path_train    = r'../../Data/SegThor_Small/train'
    input_path_test     = r'../../Data/SegThor/test'
    output_path_train   = r'../../Data/Hdf5-SegThor-channels-last/train'
    output_path_test    = r'../../Data/Hdf5-SegThor-channels-last/test'
